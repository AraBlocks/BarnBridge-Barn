...
describe('Rewards', function () {
    ...
    let barn: BarnMock, ara: Erc20Mock, rewards: Rewards;
    ...
    before(async function () {
        ...
        rewards = (await deploy.deployContract(
            'Rewards',
            [await treasury.getAddress(), ara.address, barn.address])
        ) as Rewards;
    ...
    describe('ackFunds', function () {
        it('calculates the new multiplier when funds are added', async function () {
            ...
            await ara.mint(rewards.address, amount);
        ...
        it('does not change multiplier on funds balance decrease but changes balance', async function () {
            ...
            await ara.burnFrom(rewards.address, amount.div(2));

            await expect(rewards.ackFunds()).to.not.be.reverted;
            expect(await rewards.currentMultiplier()).to.equal(helpers.tenPow18);
            expect(await rewards.balanceBefore()).to.equal(amount.div(2));

            await ara.mint(rewards.address, amount.div(2));
    ...
    describe('registerUserAction', function () {
        ...
        it('does not pull ara if function is disabled', async function () {
            await barn.callRegisterUserAction(happyPirateAddress);

            expect(await ara.balanceOf(rewards.address)).to.equal(0);

            await ara.connect(communityVault).approve(rewards.address, amount);
            ...
            const balance = await ara.balanceOf(rewards.address);
        ...
        it('does not pull ara if already pulled everything', async function () {
            ...
            expect(await ara.balanceOf(rewards.address)).to.equal(amount);
        ...
        it('updates the amount owed to user but does not send funds', async function () {
            ...
            expect(await ara.balanceOf(happyPirateAddress)).to.equal(0);
    ...
    describe('claim', function () {
        ...
        it('transfers the amount to user', async function () {
            ...
            expect(await ara.balanceOf(happyPirateAddress)).to.be.equal(expectedBalance1.add(expectedBalance2));
        ...
        it('works with multiple users', async function () {
            ...
            expect(await ara.balanceOf(rewards.address)).to.equal(expectedBalance1);
            ...
            expect(await ara.balanceOf(rewards.address)).to.equal(expectedBalance1.add(expectedBalance2));
            ...
            expect(await ara.balanceOf(rewards.address))
            ...
            expect(await ara.balanceOf(happyPirateAddress)).to.equal(expectedReward);
        });
        ...
        it('works fine after claim', async function () {
            ...
            expect(await ara.balanceOf(happyPirateAddress)).to.equal(expectedReward.add(expectedReward2));
            ...
            expect(await ara.balanceOf(flyingParrotAddress)).to.equal(expectedReward3);
        });
    });
    ...
    async function setupContracts () {
        ...
        await ara.mint(await communityVault.getAddress(), cvValue);
        await ara.mint(await treasury.getAddress(), treasuryValue);
    }
